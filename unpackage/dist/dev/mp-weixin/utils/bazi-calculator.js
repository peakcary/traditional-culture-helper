"use strict";
const utils_calendar = require("./calendar.js");
class BaziCalculator {
  ...
  calculateBazi(birthDate, gender = "男", isLunar = false) {
    ...
    if (isLunar) {
      const solarDate = utils_calendar.CalendarConverter.lunarToSolar(
    ...
    const lunarInfo = utils_calendar.CalendarConverter.solarToLunar(year, month, day);
  ...
  getDayPillar(year, month, day) {
    const ganZhi = utils_calendar.CalendarConverter.getDayGanZhi(year, month, day);
  ...
  getHourPillar(dayGan, hour) {
    const ganZhi = utils_calendar.CalendarConverter.getHourGanZhi(dayGan, hour);
    const gan = ganZhi.charAt(0);
    const zhi = ganZhi.charAt(1);
    return {
      gan,
  ...
  analyzeWuXing(bazi) {
    ...
    pillars.forEach((pillar) => {
      wuXingCount[pillar.ganWuXing] += 2;
    });
    pillars.forEach((pillar) => {
    ...
    Object.keys(wuXingCount).forEach((wx) => {
      ...
      if (wuXingCount[wx] >= total * 0.3)
        wuXingLevel[wx] = "旺";
      else if (wuXingCount[wx] >= total * 0.2)
        wuXingLevel[wx] = "强";
      else if (wuXingCount[wx] >= total * 0.1)
        wuXingLevel[wx] = "平";
      else if (wuXingCount[wx] > 0)
        wuXingLevel[wx] = "弱";
      else
        wuXingLevel[wx] = "无";
    });
    return {
      ...
      level: wuXingLevel,
      strongest: Object.keys(wuXingCount).reduce(
        (a, b) => wuXingCount[a] > wuXingCount[b] ? a : b
      ),
      weakest: Object.keys(wuXingCount).reduce(
        (a, b) => wuXingCount[a] < wuXingCount[b] ? a : b
      )
    };
  }
  ...
  getShiShenRelation(dayGan, targetGan) {
    ...
    const isSameYinYang = this.tianGan.indexOf(dayGan) % 2 === this.tianGan.indexOf(targetGan) % 2;
    if (dayWX === targetWX) {
  ...
  getWuXingRelation(wx1, wx2) {
    ...
    if (shengRelation[wx2] === wx1)
      return "生我";
    if (shengRelation[wx1] === wx2)
      return "我生";
    if (keRelation[wx2] === wx1)
      return "克我";
    if (keRelation[wx1] === wx2)
      return "我克";
    return "无关";
  }
  /**
  ...
  analyzeQiangRuo(bazi) {
    ...
    if (seasonWX === dayWX)
      strength += 3;
    else if (this.getWuXingRelation(seasonWX, dayWX) === "生我")
      strength += 1;
    else if (this.getWuXingRelation(seasonWX, dayWX) === "克我")
      strength -= 2;
    ["year", "month", "hour"].forEach((pos) => {
      const pillar = bazi.pillars[pos];
      if (this.ganWuXing[pillar.gan] === dayWX)
        strength += 2;
      if (pillar.zhiWuXing === dayWX)
        strength += 1;
    });
    let level;
    if (strength >= 5)
  ...
  analyzeGeJu(bazi) {
    ...
    const monthSS = ((_a = shiShen.month) == null ? void 0 : _a.gan) || "未知";
    const commonFormats = {
      "正官": "正官格",
      "七杀": "七杀格",
      "正财": "正财格",
...
const BaziCalculator$1 = new BaziCalculator();
exports.BaziCalculator = BaziCalculator$1;
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/bazi-calculator.js.map
