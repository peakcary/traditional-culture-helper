...
class FortuneEngine {
   ...
   */
  analyzeOverallFortune(bazi, currentYear = (/* @__PURE__ */ new Date()).getFullYear()) {
    const wuxingAnalysis = bazi.analysis.wuXing;
    const qiangRuoAnalysis = bazi.analysis.qiangRuo;
    bazi.analysis.yongShen;
    let score = 60;
    if (qiangRuoAnalysis.level === "中和")
      score += 15;
    else if (qiangRuoAnalysis.level.includes("偏"))
      score += 5;
    else if (qiangRuoAnalysis.level.includes("极"))
      score -= 10;
  ...
  calculateCategoryScore(bazi, category) {
    ...
    Object.keys(weights).forEach((wx) => {
      ...
      score += proportion * influence * 40;
    });
    const qiangRuoLevel = bazi.analysis.qiangRuo.level;
    if (qiangRuoLevel === "中和")
      score += 10;
    else if (qiangRuoLevel.includes("偏"))
      score += 5;
...
const FortuneEngine$1 = new FortuneEngine();
exports.FortuneEngine = FortuneEngine$1;
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/fortune-engine.js.map
